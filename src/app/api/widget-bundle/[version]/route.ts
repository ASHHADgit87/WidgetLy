...
(function () {
  ...
  // Edge case: guard against the same script tag firing twice (e.g. the
  // customer's site re-runs it, or a SPA remounts it) — never render the
  // same widget twice on one page.
  ...
  var THEMES = [
    { primary: '#8b5cf6', secondary: '#6366f1', accent: '#22d3ee', bgFrom: '#0f0a2e', bgTo: '#1e1147', text: '#f5f3ff', muted: '#c4b5fd' },
    { primary: '#f97316', secondary: '#ec4899', accent: '#facc15', bgFrom: '#1f0a1a', bgTo: '#3d0f24', text: '#fff7ed', muted: '#fdba74' },
    { primary: '#10b981', secondary: '#059669', accent: '#a3e635', bgFrom: '#04120d', bgTo: '#062018', text: '#ecfdf5', muted: '#6ee7b7' },
    { primary: '#3b82f6', secondary: '#6366f1', accent: '#f472b6', bgFrom: '#050b1f', bgTo: '#0b1740', text: '#eff6ff', muted: '#93c5fd' },
    { primary: '#ef4444', secondary: '#f97316', accent: '#fde047', bgFrom: '#1a0505', bgTo: '#2e0a0a', text: '#fef2f2', muted: '#fca5a5' },
    { primary: '#d946ef', secondary: '#a855f7', accent: '#67e8f9', bgFrom: '#170a29', bgTo: '#2b0f45', text: '#fdf4ff', muted: '#e9a8fd' },
  ...
  function renderInactiveMessage(mount, theme, text) {
    var box = el('div', {
      maxWidth: '360px',
      padding: '16px',
      borderRadius: '14px',
  ...
  fetch(apiBase + '/api/widgets/' + widgetId + '/config')
    ...
    .then(function (json) {
      if (!json.success) {
        renderInactiveMessage(script, theme, 'This form is no longer accepting submissions.');
        return;
      }
      ...
      form.addEventListener('submit', function (event) {
        ...
        if (submitBtn.disabled) return; // edge case: prevent double-submit
    ...
    .catch(function () {
      renderInactiveMessage(script, theme, 'Unable to load this form right now. Please refresh the page.');
    });
})();
`;
